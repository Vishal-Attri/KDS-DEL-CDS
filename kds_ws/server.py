...
def refresh_kds_cache(kds_name="NONE"):
    ...
    cached_kds_tickets[kds_name] = fetch_kds_dev_tickets(kds_name)
...
def fetch_kds_dev_tickets(kds_name="NONE"):
    try:
        conn = pyodbc.connect(CONN_STR)
        cursor = conn.cursor()
...
# ------------------- KDS_DEL UPDATE -------------------
def update_kds_dev_ticket(kot_no, bill_no, items):
    try:
        conn = pyodbc.connect(CONN_STR)
        cursor = conn.cursor()
        for item in items:
...
# ------------------- KDS_DEL BROADCAST -------------------
async def broadcast_kds_dev_tickets():
    for client in clients_kds_dev.copy():
        try:
            ...
            clients_kds_dev.discard(client)

# ------------------- KDS_DEL WEBSOCKET -------------------
async def ws_kds_dev_handler(websocket):
    clients_kds_dev.add(websocket)
    ...
    try:
        while True:
            try:
                ...
                if action == "init_kds":
                    ...
                    # Send cached tickets immediately
                ...
                if action == "toggle_ticket":
                    update_kds_dev_ticket(
                        data.get("kot_no"), data.get("bill_no"), data.get("items")
                    )
                    kds_name = client_kds_map.get(websocket, "NONE")
                    async_refresh_kds(kds_name)
                    await broadcast_kds_dev_tickets()
        ...
        clients_kds_dev.discard(websocket)
            ...
            while True:
                ...
                if row:
                    ...
                    if message_type == "KDS_TriggerMessage":
                        ...
                        loop.call_soon_threadsafe(lambda: asyncio.create_task(broadcast_kds_dev_tickets()))
